...
{
    if (m_texture == nullptr || m_streamer == nullptr || m_video == nullptr)
    {
        return;
    }
...
void app::MainScene::onInit()
{
    ...
    try
    {
        ...
        if (urlsList == config.end() || !urlsList.value().is_object() || urlsList.value().empty())
        {
        ...
        for (auto it = urlsList->begin(); it != urlsList->end(); ++it)
        {
            ...
            const auto texture = core.get<ej::TextureManager>()
                                     ->bind(textureName,
                                            [](const ej::Core &core) {
        ...
        if (transitionsList == config.end() || !transitionsList.value().is_array() || transitionsList.value().empty())
        {
        ...
        for (auto it = transitionsList->begin(); it != transitionsList->end(); ++it)
        {
            ...
            if (!transition.is_object() || transition.find("begin") == transition.end() ||
        ...
        if (firstState == config.end() || !firstState.value().is_string())
        {
...
void app::MainScene::drawVR()
{
    for (size_t i = 0; i < 2; ++i)
    {
        ...
        for (const auto &index : m_vrManager->getControllerIndices())
        {
            ...
            entity->getTransform().setTransformationMatrix(m_vrManager->getDeviceTransformation(index));
...
void app::MainScene::createMarker(const std::string &name, const glm::vec3 &position)
{
    ...
    marker->meshEntity.getTransform().scale(0.5f).move(position);
...
void app::MainScene::createLine(ej::Transform *parent)
{
    ...
    sceneEntity->meshEntity.getTransform().setParent(parent).setScale(10.0f);
...
void app::MainScene::handleControllersRaycast()
{
    for (const auto &[controllerIndex, object] : m_controllers)
    {
        ...
        void *data;
        if (m_collisionWorld.raycast(transform.getGlobalPosition(), glm::vec4(transform.getDirectionFront(), 1.0f),
                                     data))
        {
